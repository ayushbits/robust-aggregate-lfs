...
def load_spam_dataset(load_train_labels: bool = False, split_dev: bool = True):
    ...
    # if split_dev:
    df_dev = df_train.sample(55, random_state=123)
    df_valid = df_train.sample(55, random_state=123)

    # if not load_train_labels:
    #     df_train["label"] = np.ones(len(df_train["label"])) * -1
    # df_valid_test = dfs[4]
    df_train, df_test = train_test_split(df_train, test_size=500, random_state=123)
...
print('size of sets are ', len(df_train), len(df_dev), len(df_valid), len(df_test))
